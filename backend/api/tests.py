...
class RecipeTestCase(BaseTestCase):
    ...
    def setUpClass(cls):
        ...
        cls.ingredients = ('Абрикос', 'Капуста', 'Кактус', 'Качан', 'Карп',
                           'Палтус', 'Тусовка опят', 'яблоки', 'Картон',
                           'Лимон', )
        for ingredient in cls.ingredients:
            Ingredient.objects.create(
                name=ingredient,
                measurement_unit=choice(('кг', 'г', 'шт'))
            )
        ...
        for recipe in all_recipes:
            ...
            for ingredient in set(choices(
                    all_ingredients, k=randint(1, len(all_ingredients)))):
        ...
        for recipe in set(choices(
                all_recipes, k=randint(1, len(all_recipes)))):
    ...
    def test_shopping_cart(self):
        ...
        response = self.authorized.post(url)
        self.assertEqual(response.status_code, HTTPStatus.CREATED)
        self.assertTrue(Trolley.objects.filter(
            recipe_id=recipe.pk,
